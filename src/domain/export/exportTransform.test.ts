...
import { imageToScreen } from '../viewport/viewport.ts';
...
describe('export indépendant de l’écran', () => {
  it('exporte à la résolution native de la photo, quel que soit le zoom écran', () => {
    ...
    expect(imageToScreen(raster.viewport, { x: 1234, y: 5678 })).toEqual({ x: 1234, y: 5678 });
  });
  ...
  it('permet une sortie supérieure à la résolution native', () => {
    ...
    expect(imageToScreen(raster.viewport, { x: 100, y: 200 })).toEqual({ x: 150, y: 300 });
  });
