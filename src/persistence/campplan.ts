 ...
 *   fichiers/<rôle>-<sha>.<ext>   photo d'origine et PDF d'origine, octets identiques (non recompressés)
 *
...
export const CAMPPLAN_FORMAT_VERSION = 1;
...
const fileEntrySchema = z.object({
  ...
  role: z.enum(['background', 'pdf']),
  /** Identifiant du fichier dans le plan exporté (relié à `plan.baseImage`). */
...
function baseImageFiles(
  ref: BaseImageRef | null,
): { blobId: string; role: 'background' | 'pdf'; sha256: string }[] {
  if (!ref) return [];
  const files: { blobId: string; role: 'background' | 'pdf'; sha256: string }[] = [
    { blobId: ref.blobId, role: 'background', sha256: ref.sha256 },
  ];
  if (ref.source.kind === 'pdf')
    files.push({ blobId: ref.source.pdfBlobId, role: 'pdf', sha256: ref.source.pdfSha256 });
  return files;
}

function usedPresets(doc: PlanDocument): ZonePreset[] {
  const ids = new Set(
    Object.values(doc.objects)
      .map((o) => o.presetId)
      .filter(Boolean),
  );
  return AREA_PRESETS.filter((p) => ids.has(p.id));
}
...
export async function exportCampplan(
  ...
  for (const { blobId, role, sha256 } of baseImageFiles(doc.plan.baseImage)) {
    const blob = await repo.getBlob(blobId);
    if (!blob)
      throw new CampplanError(
        `Le fichier ${role === 'pdf' ? 'PDF' : 'de la photo'} est introuvable dans le stockage local.`,
      );
...
export async function readCampplan(bytes: Uint8Array): Promise<CampplanContent> {
  ...
  for (const entry of manifest.files) {
    ...
    if (data.byteLength !== entry.byteLength || (await sha256Hex(data.slice().buffer)) !== entry.sha256) {
      throw new CampplanError(
        `Le fichier ${entry.role === 'pdf' ? 'PDF' : 'de la photo'} est corrompu (empreinte SHA-256 différente).`,
      );
    }
  ...
  for (const needed of baseImageFiles(doc.plan.baseImage)) {
    const file = files.get(needed.blobId);
    if (!file || file.sha256 !== needed.sha256)
      throw new CampplanError('La photo du plan ne correspond pas aux fichiers de l’archive.');
  }
